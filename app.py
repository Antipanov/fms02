import os
from flask import Flask, render_template, request, flash, abort, redirect, url_for, json, send_file
...
values = {
    ...
    'left_fighter_d1_score': 0,
    'right_fighter_d1_score': 0,
    'left_fighter_x1_score': 0,
    'right_fighter_x1_score': 0,
    'left_fighter_m1_score': 0,
    'right_fighter_m1_score': 0,
    'competition_name': '',
    'current_fight_id':0,
...
def constractor_fighters_are_selected(comp_id, weight_cat_id, age_cat_id, round_no):
    if request.method == 'POST':
        # Получаем список getlist из формы. Мы получаем по факту два значения айдишников бойцов, которые выбраны в форме
        ...
        if len(selected_fighters) >0:
            ...
            red_fighter_reg_id = selected_fighters[0]
            red_fighter_reg_row = RegistrationDB.query.filter_by(id =red_fighter_reg_id).first()
            red_fighter_id = red_fighter_reg_row.fighter_id
            # print("red_fighter_id", red_fighter_id)
            # и присваиваем синему бойцу айдишник второго
            blue_fighter_reg_id = selected_fighters[1]
            blue_fighter_reg_row = RegistrationDB.query.filter_by(id =blue_fighter_reg_id).first()
            blue_fighter_id = blue_fighter_reg_row.fighter_id
            # Создаем новый бой с новыми бойцами
...
def fight_constructor_step2(comp_id, weight_cat_id, age_cat_id, round_no):
    ...
    # создаем список уникальных айдишников раундов из спиcка боев в fights_data_in_all_rounds
    round_list = []
    for fight in fights_data_in_all_rounds:
        if fight.round_number not in round_list:
            round_list.append(fight.round_number)

    # cоздаем словарь для таблицы История. Ключем будет круг. Значением будет список данных
...
def fight_constructor(comp_id):
    ...
    for age_cat_id, age_cat in age_cat_data.items():
        ...
        for weight_cat_id, weight_cat in weight_cat_data.items():
            ...
            regs_in_age_and_weight_cat = RegistrationDB.query.filter_by(age_cat_id=age_cat_id, weight_cat_id = weight_cat_id, competition_id = comp_id).count()
...
@app.route('/competitions/new', methods=["POST", "GET"])
...
# создание нового списочного бойца
@app.route('/fighter/new', methods=["POST", "GET"])
def fighter_new():
    form = ParticipantForm()
    if form.validate_on_submit():
        flash('Изменения сохранены')
        new_participant = FightersDB(name = form.participant_name_form.data,  last_name = form.participant_last_name_form.data,  birthday = form.birthday_form.data, active_status = 1, fighter_image_id = form.avatar_google_code.data)
    ...
    return render_template('newfighter.html', form = form)  

  
# создание возрастной категории
@app.route('/agecategory/new', methods=["POST", "GET"])
def age_category_new():
    form = AgeCategoriesForm()
    if form.validate_on_submit():
        flash('Изменения сохранены')
        new_age_category = AgecategoriesDB(sort_index = form.age_sort_index_form_field.data, age_category_name = form.age_category_name_form_field.data, age_category_start = form.age_from_form_field.data, age_category_finish = form.age_to_form_field.data)
        ...
        try:
            ...
            print(e)
            db.session.rollback()
        return render_template('newagecategory.html')
    return render_template('newagecategory.html', form = form)  
...
# редактирование возрастных категорий view
@app.route('/agecategory/<int:id>', methods=["POST", "GET"])
def age_category_edit(id):
    age_category_data = AgecategoriesDB.query.get(id)
    form = AgeCategoriesForm()
    if form.validate_on_submit():
        flash('Изменения сохранены')
        ...
        try:
            ...
            print(e)
            db.session.rollback()
        return render_template('agecategoryedit.html', form = form, age_category_data = age_category_data)
    return render_template('agecategoryedit.html', form=form, age_category_data=age_category_data)
...
def registration(competition_id):
    ...
    regs = RegistrationDB.query.filter_by(competition_id=competition_id).all()
...
def download_comp_result(filename):
    FightsDB_df = pd.read_sql_table("FightsDB", db.engine)
    FightersDB_df = pd.read_sql_table("FightersDB", db.engine)
    RoundsDB_df = pd.read_sql_table("RoundsDB", db.engine)
    weightcategoriesDB_df = pd.read_sql_table("weightcategoriesDB", db.engine)
    fight_result_df = download_comp_result_module.download_comp_result_func(FightsDB_df, FightersDB_df, RoundsDB_df, weightcategoriesDB_df)
    fight_result_df.to_excel('temp_files/comp_result.xlsx', index = False)
...
@socketio.on('Score d1 value changed')
def left_fighter_d1_score_added_func(message):
    values['left_fighter_d1_score'] = message['left_fighter_d1_score']
    #values['right_fighter_score'] = message['right_fighter_score']
    # print("values['left_fighter_d1_score'] ",values['left_fighter_d1_score'])
    emit('update_left_fighter_d1_score', message, broadcast=True)

@socketio.on('Right score d1 value changed')
def right_fighter_d1_score_added_func(message):
    values['right_fighter_d1_score'] = message['right_fighter_d1_score']
    #values['right_fighter_score'] = message['right_fighter_score']
    # print("values['left_fighter_d1_score'] ",values['left_fighter_d1_score'])
    emit('update_right_fighter_d1_score', message, broadcast=True)  

############# Обработчик кнопок х1
@socketio.on('Score x1 value changed')
def left_fighter_x1_score_added_func(message):
    values['left_fighter_x1_score'] = message['left_fighter_x1_score']
    #values['right_fighter_score'] = message['right_fighter_score']
    # print("values['left_fighter_d1_score'] ",values['left_fighter_d1_score'])
    emit('update_left_fighter_x1_score', message, broadcast=True)

@socketio.on('Right score x1 value changed')
def right_fighter_x1_score_added_func(message):
    values['right_fighter_x1_score'] = message['right_fighter_x1_score']
    #values['right_fighter_score'] = message['right_fighter_score']
    # print("values['left_fighter_d1_score'] ",values['left_fighter_d1_score'])
    emit('update_right_fighter_x1_score', message, broadcast=True)  

############# Обработчик кнопок m1
@socketio.on('Score m1 value changed')
def left_fighter_m1_score_added_func(message):
    values['left_fighter_m1_score'] = message['left_fighter_m1_score']
    #values['right_fighter_score'] = message['right_fighter_score']
    # print("values['left_fighter_d1_score'] ",values['left_fighter_d1_score'])
    emit('update_left_fighter_m1_score', message, broadcast=True)

@socketio.on('Right score m1 value changed')
def right_fighter_m1_score_added_func(message):
    values['right_fighter_m1_score'] = message['right_fighter_m1_score']
    #values['right_fighter_score'] = message['right_fighter_score']
    # print("values['left_fighter_d1_score'] ",values['left_fighter_d1_score'])
